...
export function extractDefineModuleOptionsWithDebug(wrappedContent: string): ExtractionDebug {
  ...
  try {
    ...
    if (newMatch) {
      debug.regexMatched = true;
      debug.matchedText = newMatch[0];
      debug.optionsString = `{loadNow: ${newMatch[1]}}`;
      const loadNowValue = newMatch[1] === 'true';
      debug.result = { loadNow: loadNowValue };
      return debug;
    }
    ...
    if (objectMatch) {
      debug.regexMatched = true;
      debug.matchedText = objectMatch[0];
      debug.optionsString = objectMatch[1];
      try {
        const jsonString = objectMatch[1].replace(/(\w+):/g, '"$1":');
        const options = JSON.parse(jsonString);
        if (typeof options.loadNow === 'boolean') {
          debug.result = { loadNow: options.loadNow };
          return debug;
        }
      } catch (e) {
    ...
    debug.regexMatched = debug.regexMatched || !!oldMatch;

    if (oldMatch) {
      debug.matchedText = oldMatch[0];
      debug.optionsString = oldMatch[1];

      try {
        // Convert JavaScript object literal to valid JSON
        // Replace unquoted keys with quoted keys: loadNow -> "loadNow"
        const jsonString = oldMatch[1].replace(/(\w+):/g, '"$1":');
        const options = JSON.parse(jsonString);
...
export function extractDefineModuleOptions(wrappedContent: string): ModuleOptions | null {
  try {
    ...
    if (newMatch) {
      const loadNowValue = newMatch[1] === 'true';
      console.error(`🔬 [extractDefineModuleOptions] Matched NEW format: loadNow=${loadNowValue}`);
    ...
    if (objectMatch) {
      console.error(`🔬 [extractDefineModuleOptions] Matched OBJECT-AS-2ND-PARAM format`);
      const optionsString = objectMatch[1];
      try {
        const jsonString = optionsString.replace(/(\w+):/g, '"$1":');
        const options = JSON.parse(jsonString);
        if (typeof options.loadNow === 'boolean') {
          console.error(`🔬 [extractDefineModuleOptions] Extracted loadNow=${options.loadNow} from object format`);
          return { loadNow: options.loadNow };
        }
      } catch (e) {
        console.error(`🔬 [extractDefineModuleOptions] Failed to parse object format: ${e}`);
      }
    }
    ...
    console.error(`🔬 [extractDefineModuleOptions] Old format regex match result: ${oldMatch ? 'MATCHED' : 'NO MATCH'}`);

    if (oldMatch) {
      console.error(`🔬 [extractDefineModuleOptions] Match[0] (full): ${oldMatch[0]}`);
      console.error(`🔬 [extractDefineModuleOptions] Match[1] (options): ${oldMatch[1]}`);
    ...
    console.error(`🔬 [extractDefineModuleOptions] No match found in either format - returning null`);
    return null;
...
export function wrapModuleContent(
  ...
  options?: ModuleOptions | null
    ...
    // New signature: __defineModule__(factory, loadNow, options)
    ...
    if (options === null || (options && Object.keys(options).length === 0)) {
      ...
      // loadNow=true: immediate execution
      defineCall = '__defineModule__(_main, true);';
    } else if (options?.loadNow === false) {
      // loadNow=false: explicit lazy loading
      defineCall = '__defineModule__(_main, false);';
    } else {
