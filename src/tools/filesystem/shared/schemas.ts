...
export const FILE_TYPE_SCHEMA = {
  type: 'string',
  description: 'File type (optional - auto-detected from filename/content). ⚠️ For .html files, use write({..., raw: true}) instead of write() to avoid CommonJS wrapping.',
  ...
  llmHints: {
    ...
    htmlWarning: 'NEVER use SERVER_JS for .html files - use write({..., raw: true}) or omit fileType',
...
export const CONTENT_SCHEMA = {
  ...
  llmHints: {
    ...
    htmlWarning: 'For HTML/CSS files, use write({..., raw: true}) - write() adds CommonJS wrapper that breaks HTML'
  }
} as const;
